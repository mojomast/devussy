...
class TestAnalyzeRepoCommand:
    ...
    @patch("src.cli.RepositoryAnalyzer")
    def test_analyze_repo_json_output(self, mock_analyzer_cls, tmp_path: Path) -> None:
        """Verify analyze-repo emits JSON when --json is used."""
        root = tmp_path

        analysis = MagicMock()
        analysis.root_path = root
        analysis.project_type = "python"

        structure = MagicMock()
        structure.directories = ["src"]
        structure.files = ["pyproject.toml"]
        structure.source_dirs = ["src"]
        structure.test_dirs = []
        structure.config_dirs = []
        structure.ci_dirs = []
        analysis.structure = structure

        deps = MagicMock()
        deps.manifests = [root / "pyproject.toml"]
        deps.python = ["pytest"]
        deps.node = []
        deps.go = []
        deps.rust = []
        ...
        patterns = MagicMock()
        patterns.test_frameworks = ["pytest"]
        patterns.build_tools = []
        analysis.patterns = patterns

        cfg_files = MagicMock()
        cfg_files.files = [root / "pyproject.toml"]
        analysis.config_files = cfg_files
        ...
        result = runner.invoke(app, ["analyze-repo", str(root), "--json"])
